    ...
    for (int l = 0; l < numPredDir; l++)
    {
        for (int ref = 0; ref < slice->m_numRefIdx[l]; ref++)
        {
			...
			//@@zatt - add approximate memory to reference frames
				...
				start_approx_add[l][ref] = (unsigned long long)(slice->m_refReconPicList[l][ref]->m_picBuf[0]);
				end_approx_add[l][ref] = (unsigned long long)(slice->m_refReconPicList[l][ref]->m_picBuf[0] + (stride * (maxHeight + (m_param->maxCUSize + 16) * 2)));
